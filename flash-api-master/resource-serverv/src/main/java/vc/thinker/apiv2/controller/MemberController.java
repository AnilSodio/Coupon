...
public class MemberController extends BaseController {
	...
	public SimpleResponse registered(HttpServletRequest request,
			...
			@ApiParam(value = "密码", required = false) @RequestParam("password") String password,
...
//		}
	...
	public ResponseEntity<SimpleResponse> refundDeposit(HttpServletRequest request, @UserAuthentication User user) {
		SimpleResponse resp = new SimpleResponse(messageSource, request);
		Long uid = Long.valueOf(user.getUsername());
		try {
...
//			@ApiParam(value = "语言（defaultLanguage）", required = true) 
//			@RequestParam("language") String language
	) {

		ListResponse<FeedbackTypeBO> resp = new ListResponse<>();
		List<vc.thinker.cabbage.se.bo.FeedbackTypeBO> list = feedbackService.findFeedbackTypeTypeByType(type, language);
		if (!list.isEmpty()) {
	...
	public ResponseEntity<SimpleResponse> feedback(@RequestHeader(name = "api-agent", required = false) String apiAgent,
			@ApiParam @RequestBody @Valid FeedbackVO vo, @UserAuthentication User user) {
		SimpleResponse resp = new SimpleResponse();
		Long uid = Long.valueOf(user.getUsername());
		try {
			logger.info("vostypeId" + vo.getTypeId());
	...
	@ApiOperation(value = "问题反馈")
	@ApiResponses(value = { @ApiResponse(code = 406, message = "业务操作错误") })
	public ResponseEntity<SimpleResponse> support(@RequestHeader(name = "api-agent", required = false) String apiAgent,
			@ApiParam @RequestBody @Valid FeedbackVO vo, @UserAuthentication User user ) {
		SimpleResponse resp = new SimpleResponse();
		Long uid = Long.valueOf(user.getUsername());
		try {
...
//		Long uid = Long.valueOf(user.getUsername());
	...
	public ResponseEntity<SingleResponse<PayDetailsBO>> paymetPb(HttpServletRequest request,
			@UserAuthentication User user,
//			@ApiParam(value = "成功回调地址", required = false) @RequestParam(value="returnUrl",required=false) String returnUrl,
			@ApiParam(value = "支付方式  alipay_app,wx_app,wx_js,stripe,cashfree", required = true) @RequestParam(value = "paymentMark", required = true, defaultValue = PaymentConstants.PAYMENT_MARK_WX_JS_PAY) String paymentMark) {
...
//			memberService.saveReturnUrl(uid, returnUrl, jedisPool);
			...
			DirectPayResponse payResponse = memberService.createPbBuyPay(uid, payCallback, paymentMark);
			if (!payResponse.isSuccess()) {
				resp.setErrorInfo("406", payResponse.getMsg());
				return Responses.ok(resp);
			}
